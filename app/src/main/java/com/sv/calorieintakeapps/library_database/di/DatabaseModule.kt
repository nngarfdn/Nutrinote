...
import com.sv.calorieintakeapps.BuildConfig.URT_BASE_URL
import com.sv.calorieintakeapps.BuildConfig.URT_PUBLIC_KEY
...
val networkModule = module {
    single {
        ...
        val authInterceptor = Interceptor { chain ->
            val request = chain.request().newBuilder()
                .addHeader("Authorization", "Bearer $URT_PUBLIC_KEY")
                .build()
    ...
    single {
        Retrofit.Builder()
            .baseUrl(URT_BASE_URL)
            .addConverterFactory(MoshiConverterFactory.create())
            .client(get())
            .build()
            .create(UrtApiService::class.java)
    }
}
